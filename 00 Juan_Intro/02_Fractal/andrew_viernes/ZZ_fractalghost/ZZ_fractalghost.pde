...
void draw() {
  //background(255);
  //fill(255,10);
  float ca = sin(angle/20 + 15);
  float cb = cos(angle + time);

  float w = abs(sin(angle))*0.2+2;
  float h = (w * height) / width/1.8;
  ...
  for (int j = 0; j < height; j++) {
    ...
    for (int i = 0; i < width; i++) {
      ...
      if (n == maxiterations) {
        pixels[i+j*width] = color(127,255,25,5);
      } else {
        pixels[i+j*width] = color(127, n*16 % 255, 255,random(50));
      }
      x += dx;
    }
    y += dy;
  ...
  //saveFrame("01_output/ghost_####.png");
}
